...
export default function BusinessOrderViewer() {
  ...
  const order = useMemo(
    () =>
      loadServiceRequests().find(
        (r) => r.id === orderId && user && r.requesterOrgId === user.orgId,
      ),
    [orderId, user],
  );
      ...
      {!isDelivered ? (
        <section className="saas-card text-center py-10">
          ...
          <p className="text-xs text-ink/55 mt-2">
            Current status: <strong>{order.status.replaceAll("-", " ")}</strong>
          </p>
        </section>
      ) : (
        <>
...
function downloadAsText(order: ServiceRequest) {
  ...
  a.href = url;
  a.download = `${order.serviceTitle.replaceAll(/\s+/g, "-").toLowerCase()}-${order.id}.txt`;
...
// Same lightweight Markdown renderer used on the counselor side.
