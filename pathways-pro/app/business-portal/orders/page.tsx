...
export default function BusinessServiceOrdersPage() {
  ...
  const orders = useMemo(() => {
    ...
    return loadServiceRequests()
      .filter((r) => r.requesterOrgId === user.orgId)
      .sort((a, b) => {
        // Delivered first (so they see results), then by requested date desc
  ...
  const filtered = useMemo(() => {
    ...
    return orders.filter((o) => o.status !== "delivered" && o.status !== "declined");
  }, [orders, tab]);
...
function OrderRow({ order }: { order: ServiceRequest }) {
  ...
  const href = isDelivered
    ? `/business-portal/orders/${order.id}`
    : `/business-portal/orders/${order.id}`;
