"use client";

import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";
import { loadSession } from "@/lib/session";
import { CLIENT_RESOURCES, totalClientTools } from "@/lib/pathways-resources";

export default function ClientResourcesPage() {
  const router = useRouter();
  const [authorized, setAuthorized] = useState(false);
  const [query, setQuery] = useState("");

  useEffect(() => {
    const s = loadSession();
    if (!s) {
      router.replace("/");
      return;
    }
    setAuthorized(true);
  }, [router]);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return CLIENT_RESOURCES;
    return CLIENT_RESOURCES.map((cat) => ({
      ...cat,
      items: cat.items.filter(
        (i) =>
          i.name.toLowerCase().includes(q) ||
          i.source.toLowerCase().includes(q) ||
          i.desc.toLowerCase().includes(q),
      ),
    })).filter((c) => c.items.length > 0);
  }, [query]);

  if (!authorized) return null;

  return (
    <div className="space-y-6">
      <header>
        <p className="text-xs uppercase tracking-widest text-ink/50 mb-1">
          Helpful Resources
        </p>
        <h1 className="text-4xl mb-2">
          {totalClientTools} tools to help you find work, training, and support.
        </h1>
        <p className="text-ink/70 prose-narrow">
          Every link goes to a free, government-verified or trusted nonprofit source.
        </p>
      </header>

      <input
        type="search"
        value={query}
        onChange={(e) => setQuery(e.target.value)}
        placeholder="Search resources…"
        className="w-full bg-cream border border-ink/15 rounded-lg px-4 py-3 focus:outline-none focus:border-accent"
      />

      <div className="space-y-10">
        {filtered.map((cat) => (
          <section key={cat.category} id={`cat-${cat.category.replace(/\s+/g, "-")}`}>
            <div className="flex items-baseline justify-between mb-4 pb-2 border-b border-ink/10">
              <h2 className="text-2xl">
                {cat.icon} {cat.category}
              </h2>
              <span className="text-xs uppercase tracking-wider text-ink/40">
                {cat.items.length} resource{cat.items.length === 1 ? "" : "s"}
              </span>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              {cat.items.map((r) => (
                <a
                  key={r.name}
                  href={r.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="border border-ink/10 rounded-lg p-4 bg-cream hover:border-accent transition flex flex-col gap-1"
                >
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="font-semibold">{r.name}</span>
                    <span className="text-[10px] uppercase tracking-wider text-accent">
                      {r.source}
                    </span>
                  </div>
                  <p className="text-sm text-ink/70">{r.desc}</p>
                </a>
              ))}
            </div>
          </section>
        ))}
        {filtered.length === 0 && (
          <p className="text-center text-ink/50 py-12">No resources match your search.</p>
        )}
      </div>
    </div>
  );
}
