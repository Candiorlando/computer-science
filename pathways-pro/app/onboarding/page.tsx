...
import { ENTITY_TYPES, recommendedFeatures } from "@/lib/ecosystem-profiles";
...
export default function OnboardingPage() {
  ...
  const featurePreview = useMemo(
    () => recommendedFeatures({ entityType, specialty: rehabilitationTitle, businessServices, employmentPartnerOptIn: employmentPartner }),
    [entityType, rehabilitationTitle, businessServices, employmentPartner],
  );
...
function Textarea({ label, value, setValue, placeholder }: { label: string; value: string; setValue: (x: string) => void; placeholder: string }) { return <label className="space-y-1 block"><span className="text-xs font-bold text-ink/60">{label}</span><textarea value={value} onChange={(e) => setValue(e.target.value)} placeholder={placeholder} className="onb-input min-h-[110px]" /></label>; }
function Select({ label, value, setValue, options, error }: { label: string; value: string; setValue: (x: string) => void; options: { value: string; label: string }[]; error?: string }) { return <label className="space-y-1 block"><span className="text-xs font-bold text-ink/60">{label}</span><select value={value} onChange={(e) => setValue(e.target.value)} className="onb-input"><option value="">Select…</option>{options.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}</select>{error && <span className="text-xs text-red-600">{error}</span>}</label>; }
